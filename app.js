...
const els = {
  ...
  sourceForm: $("sourceForm"), sourceFile: $("sourceFile"), sourceText: $("sourceText"), draftInput: $("draftInput"), highlightedDraft: $("highlightedDraft"),
...
async function readSourceFile() {
  ...
  if (!/\.(txt|md|csv|html|htm|json|rtf)$/i.test(file.name)) { alert("This version reads text-like files only. For PDF or Word files, copy and paste the useful text into the source text box."); return; }
...
function addSource(event) {
  ...
  if (!text) { alert("Add source text so the checker can search it."); return; }
  state.sources.push({
    ...
    publisher: val("sourcePublisher"), url: val("sourceUrl"), pages: val("sourcePages"), text, created: new Date().toISOString()
  });
  save(); event.target.reset(); renderModuleFilter(); renderLibrary(); analyseDraft(false);
}
...
function renderLibrary() {
  ...
  const list = state.sources.filter(s => (!mod || s.module === mod) && (!q || norm([s.module, s.unit, s.title, s.authors, s.year, s.publisher, s.url, s.pages, s.text].join(" ")).includes(q)));
...
function findSourcesFor(term) { const n = norm(term); if (!n) return []; return state.sources.map(s => norm(s.text).includes(n) ? { source: s, score: n.split(" ").length, snippet: snippetAround(s.text, term) } : null).filter(Boolean).sort((a, b) => b.score - a.score).slice(0, 8); }
...
function showMatches(key, start, end) {
  ...
  els.matchPanel.innerHTML = `<p><span class="citation-chip">${esc(key)}</span> matched ${found.length} source(s).</p>` + found.map(m => `<div class="match-item"><div class="match-title">${esc(m.source.title)}</div><div class="source-meta">${esc(m.source.module)} • ${esc(m.source.unit)} • ${esc(m.source.authors)} (${esc(m.source.year)})</div><div class="snippet">${m.snippet}</div><div class="source-actions"><button data-add="${m.source.id}">Add citation ${esc(inlineCitation(m.source))}</button><button class="ghost" data-copy="${m.source.id}">Copy ref</button><button class="ghost" data-ignore="${escAttr(key)}">Ignore this term</button></div></div>`).join("");
  els.matchPanel.querySelectorAll("[data-add]").forEach(b => b.onclick = () => addCitation(start, end, b.dataset.add));
  els.matchPanel.querySelectorAll("[data-copy]").forEach(b => { const s = sourceById(b.dataset.copy); b.onclick = () => copyText(harvardRef(s)); });
  els.matchPanel.querySelectorAll("[data-ignore]").forEach(b => b.onclick = () => ignoreTerm(b.dataset.ignore));
}

function addCitation(start, end, sourceId) { if (!state.citations.find(c => c.start === start && c.end === end && c.sourceId === sourceId)) state.citations.push({ id: id(), start, end, sourceId }); save(); renderCitedDraft(); }
function renderCitedDraft() {
  ...
  let out = text; cites.forEach(c => { out = out.slice(0, c.end) + ` ${inlineCitation(sourceById(c.sourceId))}` + out.slice(c.end); });
...
function harvardRef(s) { const pub = s.publisher ? ` ${s.publisher}.` : ""; const loc = s.pages ? ` ${s.pages}.` : ""; const url = s.url ? ` Available at: ${s.url}.` : ""; return `${s.authors || "Unknown author"} (${s.year || "n.d."}) ${s.title || "Untitled source"}.${pub}${loc}${url}`.replace(/\s+/g, " ").trim(); }
...
function snippets(text, q, count = 3) { const n = norm(text), needle = norm(q); let idx = 0, out = []; while (out.length < count && (idx = n.indexOf(needle, idx)) > -1) { out.push(snippetAround(text, q)); idx += Math.max(needle.length, 1); } return out; }
function snippetAround(text, term) { const lower = text.toLowerCase(), n = String(term).toLowerCase(); let idx = lower.indexOf(n); if (idx < 0) idx = Math.max(0, norm(text).indexOf(norm(term))); const start = Math.max(0, idx - 110); const end = Math.min(text.length, idx + String(term).length + 130); let sn = esc(text.slice(start, end)); const safe = esc(term).replace(/[.*+?^${}()|[\]\\]/g, "\\$&"); try { sn = sn.replace(new RegExp(safe, "ig"), m => `<mark>${m}</mark>`); } catch {} return `${start > 0 ? "…" : ""}${sn}${end < text.length ? "…" : ""}`; }
